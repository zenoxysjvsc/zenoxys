let handler = async (m, { conn, text, usedPrefix, isAdmin, isOwner }) => {
    ...
    let id = m.chat
    if (id in conn.vote) return conn.sendButton(m.chat, 'Masih ada vote di chat ini!', '© Zenoxys', 'Hapus', `${usedPrefix}-vote`, m)
...
*${usedPrefix}hapusvote* - untuk menghapus vote`, '© Zenoxys', 'Upvote', `${usedPrefix}upvote`, 'Devote', `${usedPrefix}devote`, m)
    conn.vote[id] = [
        text,
        [],
        []
