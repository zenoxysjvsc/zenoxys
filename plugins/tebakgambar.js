...
`.trim()
  conn.tebakgambar[id] = [
    await conn.sendButtonImg(m.chat, json.img, caption, '© Zenoxys', 'Bantuan', '.hint', m),
    json, poin,
    setTimeout(() => {
      if (conn.tebakgambar[id]) conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.jawaban}*`, '© Zenoxys', 'Tebak Gambar', '.tebakgambar', conn.tebakgambar[id][0])
