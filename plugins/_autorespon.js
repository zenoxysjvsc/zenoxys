    ...
    if (m.isGroup) {
        if (m.mentionedJid.includes(this.user.jid)) {
            await this.send2Button(m.chat,
                isBanned ? 'Zenoxys tidak aktif' : banned ? 'kamu dibanned' : 'Zenoxys aktif',
                '© Zenoxys',
...
contact me:
http://wa.me/6281528745589?text=saya%20ingin%20bertanya%20tentang%20sewa%20bot
...
`.trim(), '© Zenoxys', 'Pemilik Bot', ',owner', m)
    ...
    if (set.autoupdatestatus) {
        if (new Date() * 1 - set.status > 1000) {
            ...
            await this.setStatus(`Aktif selama ${uptime} | Mode: ${set.self ? 'Private' : set.group ? 'Hanya Grup' : 'Publik'} | Zenoxysbot oleh zyhkss`).catch(_ => _)
